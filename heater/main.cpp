...
namespace MockRoom {
    ...
    constexpr float kHeatingRateCPerMs = 0.00004f;
    ...
    void applySteps(int8_t steps) {
        heaterSetpointC += steps * 0.5f;
        if (heaterSetpointC < 16.0f) heaterSetpointC = 16.0f;
        if (heaterSetpointC > 30.0f) heaterSetpointC = 30.0f;
    }
...
void onHeaterTurnedOff(uint32_t nowMs, const WallClockSnapshot& wallNow,
    ...
    Serial.printf("[HEAT] OFF at %02d:%02d — ran %um%02us | %.1f°C → %.1f°C (%.1f° rise) | target was %.1f°C\n",
                  wallNow.hour, wallNow.minute,
                  durationMin, durationSec,
                  gRoomAtOnC, roomTempC, rise,
                  gTargetAtOnC);
...
void setup() {
    ...
    Serial.printf("[MOCK] Room model: start=%.1f°C outside=%.1f°C target=%.1f°C\n",
...
void loop() {
    ...
    // ── 5. Adaptive tuning ────────────────────────────────────
    const AdaptiveThermostatTuning::Overrides overrides =
        gAdaptive.update(nowMs, roomTempC, gPid.mode(),
                         gPid.baseTuningForMode(gPid.mode()));
